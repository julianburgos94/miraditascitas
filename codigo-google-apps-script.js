...
function doGet(e) {
  try {
    ...
    if (accion === 'guardar') {
      ...
      // La hora se guarda con apóstrofe invisible para forzar texto plano
      // y evitar que Google Sheets la convierta a formato de tiempo
      hoja.appendRow([id, fechaRegistro, nombre, telefono, servicio, profesional, fecha, hora, 'pendiente']);

      // Forzar que la columna de Hora (col 8) sea texto plano en esta fila
      const ultimaFila = hoja.getLastRow();
      hoja.getRange(ultimaFila, 8).setNumberFormat('@');  // @ = formato texto
