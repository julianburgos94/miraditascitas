...
const CABECERAS = [
  ...
  'Servicio', 'Profesional', 'Fecha Cita', 'Hora', 'Estado'
];
...
function doPost(e) {
  try {
    ...
    const id = 'MIR-' + Math.random().toString(36).substr(2, 6).toUpperCase();
    const fechaRegistro = Utilities.formatDate(new Date(), 'America/Bogota', 'yyyy-MM-dd HH:mm:ss');
    hoja.appendRow([id, fechaRegistro, datos.nombre, datos.telefono, datos.servicio, datos.profesional, datos.fecha, datos.hora, 'pendiente']);
...
function obtenerHorariosOcupados(hoja, fecha, profesional) {
  ...
  // datos[0] son las cabeceras, datos[1..n] son las citas

  // Índices de columnas (basado en el orden de CABECERAS)
  // Col 0=ID, 1=FechaReg, 2=Nombre, 3=Tel, 4=Servicio, 5=Profesional, 6=FechaCita, 7=Hora, 8=Estado
  ...
  for (let i = 1; i < datos.length; i++) {  // i=1 para saltar cabeceras
    const fila = datos[i];

    // Normalizar la fecha (puede venir como Date object si Google la convirtió)
    ...
    const filaProfesional = String(fila[COL_PROFESIONAL]).trim();
    const filaHora        = String(fila[COL_HORA]).trim();
    const filaEstado      = String(fila[COL_ESTADO]).trim().toLowerCase();

    // Una cita ocupa el horario si:
    // - coincide la fecha
    // - coincide la profesional
    // - NO está cancelada
    if (
      fechaCita       === fecha &&
      filaProfesional === profesional &&
