 ...
 */
const SHEETS_URL = 'https://script.google.com/macros/s/AKfycbwe2MjE37KdbZOrVJhvX6kfh-PUToDXDE8FyTuJbCFM5BE1gldiagWqjiE-o6XtY5ld/exec';
...
const EQUIPO = [
  {
    ...
    especialidad: 'Extensiones & Lifting',
    experiencia:  '5 años',
    desc:         'Artista de la mirada con certificación internacional en volumen ruso y mega volumen.',
    emoji:        '💁',
    foto:         'fotos/valentina.jpg',
    tags:         ['Extensiones', 'Lifting', 'Tinte']
  },
  {
    nombre:       'Camila Torres',
    cargo:        'Nail Artist',
    especialidad: 'Uñas & Nail Art',
    experiencia:  '4 años',
    desc:         'Diseñadora de uñas con técnica en gel, acrílico y los más increíbles diseños artísticos.',
    emoji:        '👩‍🎨',
  ...
  },
  {
    ...
    cargo:        'Especialista Facial & Spa',
    especialidad: 'Faciales & Corporal',
    experiencia:  '6 años',
    desc:         'Esteticista certificada experta en limpiezas profundas, tratamientos antiedad y corporales.',
    emoji:        '🧖‍♀️',
    foto:         'fotos/daniela.jpg',
    tags:         ['Facial', 'Spa', 'Masajes']
  },
  {
    nombre:       'Laura Jiménez',
    cargo:        'Maquilladora & PMU',
    especialidad: 'Micropigmentación',
    experiencia:  '7 años',
    desc:         'Artista de micropigmentación y maquillaje profesional para novias y eventos especiales.',
    emoji:        '💄',
    foto:         'fotos/laura.jpg',
    tags:         ['Maquillaje', 'PMU', 'Cejas']
  }
];
...
async function guardarCitaEnSheets(cita) {
  ...
  try {
    ...
    // Si falla la conexión, dejamos continuar con ID local para no bloquear al cliente
    return { ok: true, id: 'MIR-' + Math.random().toString(36).substr(2, 6).toUpperCase(), sinConexion: true };
  }
}
...
function renderServicios() {
  ...
  contenedor.innerHTML = SERVICIOS.map((s, idx) => `
...
function renderEquipo() {
   ...
   * Si la foto no existe o falla, muestra el emoji de respaldo.
   */
  const cardHTML = p => {
    // Contenido del avatar: imagen real con respaldo en emoji
    const avatar = p.foto
    ...
    return `
      <div class="card-equipo">
        ...
        <button class="btn-agendar-pro"
          onclick="event.stopPropagation(); irAgendar('', ${JSON.stringify(p.nombre)})">
  ...
  // Duplicar 4 veces para el carrusel infinito sin saltos visibles
...
async function actualizarHorasDisponibles() {
  ...
  selHora.innerHTML = '<option value="">— Selecciona hora —</option>' +
    HORARIOS.map(h => {
      ...
      return `<option value="${h}" ${estaOcupado ? 'disabled' : ''}>
        ${estaOcupado ? `${h}  ❌ Ocupado` : `${h}  ✅ Disponible`}
      </option>`;
    }).join('');
...
document.getElementById('btn-agendar').addEventListener('click', async function () {
  ...
  try {
    const cita = { nombre, telefono, servicio, profesional, fecha, hora };
